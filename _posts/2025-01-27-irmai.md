...
The Turing test is dead. We will never know if we are talking to an AI or not. Unless we start having doubt and try to jailbreak it... The first stage of the LLM revolution was the chatbot phase that builds on the linguistic capabilities of LLMs to deliver general-purpose information to humans. Then came the second stage, where chatbots would take on any role or personality, famous or not (hello [character.ai](character.ai)). The next stage is to craft that AI role-play to perfectly represent anyone, not just a prototype character. 
  

![internet dogs](/assets/images/internet_dog.jpg)
  

After our initial exploration of using LLMs to simulate human behaviors with GSS data (cf EARLIER POST), we asked ourselves: how could this have practical application in the B2B world? WWhere does one need non-expert opinion whether it is for launching a new line of cosmetic product, a video game or TV show?

## Consumer Intelligence at Scale

This is how we opened the door to the world of "Consumer Research & Consummer Intelligence". After a quick tour of the industry (TOO BAD I DIDNT HAVE O1/O3 at the time to conduct that research), we identified 3 main current players:
