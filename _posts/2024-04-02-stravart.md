...
We now have all the pieces to search for the best riddeable path representing our image contour. The parameter space to search over is made of: 
...
All the above experiments made it into a Python package I've developed[^3] called [`stravart`](https://github.com/dsleo/stravart). Every step of this post is detailled in a separate [notebooks](https://github.com/dsleo/stravart/tree/main/notebooks) (whether contour extraction or hyperparameter optimization). There is even a small [streamlit webapp](https://github.com/dsleo/stravart/blob/main/app/app.py) that displayed the optimization process on a map. I initially wanted to make this available as a service, deployed on a website but I haven't managed to get to it yet...
